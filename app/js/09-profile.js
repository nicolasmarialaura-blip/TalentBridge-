...
function setCPAvatar(e){var f=e.target.files[0];if(!f)return;compressImage(f,500,0.85,function(dataUrl){if(!dataUrl)return;cpData.avatar=dataUrl;cpData.photo=dataUrl;renderCPAvatar();});}
function setCPBanner(e){var f=e.target.files[0];if(!f)return;compressImage(f,1200,0.85,function(dataUrl){if(!dataUrl)return;cpData.banner=dataUrl;var wrap=G('cp-banner-wrap');if(wrap){wrap.style.backgroundImage='url('+dataUrl+')';wrap.style.backgroundSize='cover';wrap.style.backgroundPosition='center';var sp=wrap.querySelector('span');if(sp)sp.style.opacity='0';}});}
...
function addGalleryPhotos(e){var files=Array.from(e.target.files).slice(0,9-cpData.gallery.length);files.forEach(function(f){compressImage(f,1000,0.85,function(dataUrl){if(!dataUrl)return;cpData.gallery.push(dataUrl);renderGallery();});});e.target.value='';}
